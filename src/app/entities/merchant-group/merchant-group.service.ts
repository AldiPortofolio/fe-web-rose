...
import { SERVER_PATH, SERVER, PATH_IMAGES, SERVER_GO } from 'src/app/shared/constants/base-constant';
...
export class MerchantGroupService {
    ...
    private serverGo = SERVER_GO + 'merchantGroup';
    ...
    findConfigVa(id: number): Observable<HttpResponse<MerchantGroupVaInfo>> {
        return this.http.get<MerchantGroupVaInfo>(`${SERVER_GO}config-va-merchant-group/find-by-group-id/${id}`, { observe: 'response'})
            .pipe(map((res: HttpResponse<MerchantGroupVaInfo>) => this.convertResponseConfigVa(res)));
    }

    // find(id: number): Observable<EntityResponseType> {
    //     return this.http.get<MerchantGroup>(`${this.serverGo}/id/${id}`, { observe: 'response' })
    //         .pipe(map((res: EntityResponseType) => this.convertResponse(res)));
    // }
    ...
    saveConfigVa(merchantGroupVaInfo: MerchantGroupVaInfo): Observable<HttpResponse<MerchantGroupVaInfo>> {
    
        return this.http.post<MerchantGroupVaInfo>(`${SERVER_GO}config-va-merchant-group/save`, merchantGroupVaInfo, { observe: 'response'})
            .pipe(map((res: HttpResponse<MerchantGroupVaInfo>) => this.convertResponseConfigVa(res)));
    }

    saveConfigVaRedis(merchantGroupVaInfo: MerchantGroupVaInfo): Observable<HttpResponse<MerchantGroupVaInfo>> {
    
        return this.http.post<MerchantGroupVaInfo>(`${SERVER_GO}config-va-merchant-group/save-to-redis`, merchantGroupVaInfo, { observe: 'response'})
            .pipe(map((res: HttpResponse<MerchantGroupVaInfo>) => this.convertResponseConfigVa(res)));
    }
    ...
    private convertResponseConfigVa(res: HttpResponse<MerchantGroupVaInfo>): HttpResponse<MerchantGroupVaInfo> {
        console.log('convert response');
        const body: MerchantGroupVaInfo = this.convertVaConfigFromServer(res.body);
        return res.clone({body});
    }
    ...
    private convertVaConfigFromServer(merchantGroupVaInfo: MerchantGroupVaInfo): MerchantGroupVaInfo {
      const copyOb: MerchantGroupVaInfo = Object.assign({}, merchantGroupVaInfo);
      return copyOb;
    }
    ...
    getConfigVaRedis(id): Observable<HttpResponse<MerchantGroupVaInfo>> {
        const newresourceUrl = SERVER_GO  + "config-va-merchant-group"+ `/find-by-group-id-from-redis/${id}`;
        return this.http.get<MerchantGroupVaInfo> (newresourceUrl, { observe: 'response'})
            .pipe(
                tap(result => console.log('hasil', result))
            );
    }
