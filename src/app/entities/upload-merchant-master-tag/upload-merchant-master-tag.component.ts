...
@Component({
  selector: 'op-upload-merchant-master-tag',
  templateUrl: './upload-merchant-master-tag.component.html',
  styleUrls: ['./upload-merchant-master-tag.component.css']
})
export class UploadMerchantMasterTagComponent implements OnInit {

    reportList: UploadMerchantMasterTag[];
    curPage = 1;
    totalData = 0;
    totalRecord = 10;
    searchTerm = {
    ...
    uploadNmidRes: UploadMerchantMasterTag;

    constructor(
        private ngxService: NgxUiLoaderService,
        private uploadMerchantService: UploadMerchantMasterTagService,
    ...
    loadAll(page) {
        ...
        this.uploadMerchantService.filter({
            ...
            .subscribe(
                (res: HttpResponse<UploadMerchantMasterTag[]>) => this.onSuccessGetData(res.body),
    ...
    submitUpload() {
        ...
        this.uploadMerchantService.uploadFile(formData)
            .subscribe(
                (res: HttpResponse<UploadMerchantMasterTag>) => this.onSuccess(res),
                (res: HttpErrorResponse) => this.onError(res.message),
            )
    }
