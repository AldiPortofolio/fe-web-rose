...
import { WilayahConfiguration } from '../wilayah-configuration/wilayah-configuration.model';
...
import { WilayahConfigurationService } from '../wilayah-configuration/wilayah-configuration.service';
...
export class MasterDataApprovalDetailComponent implements OnInit {
    ...
    constructor(private masterDataApprovalService: MasterDataApprovalService,
            ...
            private wilayahConfigurationService: WilayahConfigurationService,
    ...
    merchantGroupVaInfoOld: MerchantGroupVaInfo;
    merchantGroupVaInfoNew: MerchantGroupVaInfo;
    regionList: Region[];
    areaList: Area[];
    branchList: Branch[];
    ...
    ngOnInit() {
        ...
        console.log('Module idMda --> ', this.idMda);
        ...
        switch (this.moduleName) {
            ...
            case 'Module System Parameter':
                ...
                break;
            case 'Module Wilayah Config':
                this.loadDataWilayahConfig();
                break;
            case 'Module Merchant Aggregator':
                this.loadDataMerchantAggregator();
                break;
        }

    }

    loadDataWilayahConfig() {
        // start loader wilayah config
        this.ngxService.start();
        this.wilayahConfigurationService.getFromMda(this.idMda)
            .subscribe(
    ...
    loadDataMerchantAggregator() {
        // start loader
        this.ngxService.start();

        this.merchantAggregatorService.getFromMda(this.idMda)
            .subscribe(
    ...
    private onSuccessConfigVa(data, headers) {
        ...
        if (data.errCode != "00") {
            this.merchantGroupVaInfoOld = new MerchantGroupVaInfo();
            return;
        }
        this.merchantGroupVaInfoOld = data.data;
    }

    private onSuccessConfigVaRedis(data, headers) {
        this.ngxService.stop();
        console.log("ini responconfigva redis", data)
        if (data.errCode != "00") {
            return;
        }
        this.merchantGroupVaInfoNew = data.data;
        console.log("ini responconfigva redis setelah di assign",this.merchantGroupVaInfoNew)
    }
    ...
    private onSuccess(data, headers) {
        ...
        if (this.recAction === MDA_ACTION_REPAIR) {
            this.repair = true;
            console.log("masuk reapir");
            this.statusDisable = 'false';
        }
        if (this.moduleName === 'Module Wilayah Config') {
            this.wilayahConfig = data[1];

            if (data[1].status == 0) {
                this.statusWilayahConfig = 'delete';
            }
            
            console.log('data --> ', this.wilayahConfig);
            console.log('status --->', this.statusWilayahConfig);
        }
        ...
        if (this.moduleName === 'Module Merchant Group') {
            ...
            this.ngxService.start()
            this.merchantGroupService.findConfigVa(this.merchantGroupList[1].id)
                .subscribe(
                    (res: HttpResponse<MerchantGroupVaInfo>) => this.onSuccessConfigVa(res.body, res.headers),
                    (res: HttpErrorResponse) => this.onError(res.message),
                    () => { console.log('finale'); }
                );
            this.ngxService.start()
            this.merchantGroupService.getConfigVaRedis(this.merchantGroupList[1].id)
                .subscribe(
                    (res: HttpResponse<MerchantGroupVaInfo>) => this.onSuccessConfigVaRedis(res.body, res.headers),
                    (res: HttpErrorResponse) => this.onError(res.message),
                    () => { console.log('finale'); }
                );
        }
        ...
        if (this.moduleName === 'Module Merchant Aggregator') {
            this.merchantAggregatorList = data;
            if (this.merchantAggregatorList[0] === null) {
                this.merchantAggregatorList[0] = new MerchantAggregator();
            }
        }
    ...
    onConfirm() {
        switch (this.moduleName) {
            ...
            case 'Module Merchant Group':
                ...
                break;
            case 'Module Wilayah Config':
                this.saveWilayahConfiguration();
                break;
            case 'Module Merchant Aggregator':
                this.saveMerchantAggregator();
                break;
        }
    }
            ...
            .subscribe(
                (res: MasterDataApproval) => {
                    if (res.errCode === '00') {
                        ...
                        Swal.fire('Success', 'Reject success!', 'success').then(
                            result => this.onBack()
                        );
                    }
                },
    ...
    requestRepair() {
        this.masterDataApprovalService.requestRepair(this.idMda)
            .subscribe(
                (res: MasterDataApproval) => {
                    if (res.errCode === '00') {
                        Swal.fire('Success', 'Request Repair!', 'success').then(
                            result => this.onBack()
                        );
                    }
                },
    ...
    saveMerchantGroup(): void {
        // start loader
        this.ngxService.start();
        console.log('DATA YANG TERSIMPAN dan Id ==>', this.merchantGroupList[1], this.idMda );
        console.log("DATA CONFIG VA", this.merchantGroupVaInfoNew)
        this.merchantGroupService.saveConfigVa(this.merchantGroupVaInfoNew)
            .subscribe (
                res => console.log("Sukses save configVA to DB",res),
                () => console.log("error save config VA"),
                () => this.ngxService.stop()
            )
        this.ngxService.start();
            ...
            .subscribe(
                (res: MerchantGroup) => {
                    ...
                    if (res.errCode === '00') {
                        ...
                        Swal.fire('Success', 'Success Save Merchant Group!', 'success').then(
                            result => this.onBack()
                        );
                    }
                    // stop loader
    ...
    saveWilayahConfiguration(): void {
        // start loader
        this.ngxService.start();
        this.wilayahConfigurationService.approveFromMda(this.idMda)
            .subscribe(
                (res: WilayahConfiguration) => {
                    console.log(res);
                    if (res.errCode === '00') {
                        console.log('save');
                        this.onBack();
    ...
    saveMerchantAggregator(): void {
        // start loader
        this.ngxService.start();
        this.merchantAggregatorService.approveFromMda(this.idMda)
            .subscribe(
                (res: MerchantAggregator) => {
                    if (res.errCode === '00') {
                        this.onBack();
                    }
                    // stop loader
