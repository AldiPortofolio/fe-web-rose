...
import { ReportUpdatedDataMerchant } from './report-updated-data-merchant.model';
...
@Component({
  selector: 'op-report-updated-data-mechant',
  templateUrl: './report-updated-data-mechant.component.html',
  styleUrls: ['./report-updated-data-mechant.component.css']
})
export class ReportUpdatedDataMechantComponent implements OnInit {

    reportList: ReportUpdatedDataMerchant[];
    curPage = 1;
    totalData = 0;
    ...
    constructor(private ngxService: NgxUiLoaderService,
        private reportCrmPenugasanService: ReportUpdatedDataMechantService) { }
    ...
    loadAll(page) {
        ...
        this.reportCrmPenugasanService.filter({
            startDate: '',
            endDate: '',
            page: page,
            ...
            .subscribe(
                (res: HttpResponse<ReportUpdatedDataMerchant[]>) => this.onSuccessGetData(res.body),
    ...
    downloadFile(filePath) {
        ...
        this.reportCrmPenugasanService.download({
            filePath: filePath
        }).then(
            (resp) => {
    ...
    sendKafka() {
        ...
        this.reportCrmPenugasanService.sendKafka({
            startDate: this.searchTerm.startDate,
            endDate: this.searchTerm.endDate
        })
            .subscribe(
                (res: HttpResponse<ReportUpdatedDataMerchant[]>) => this.onSuccess(res.body),
                (res: HttpErrorResponse) => this.onError(res.message),
                () => { console.log('finally'); }
            );
    }
