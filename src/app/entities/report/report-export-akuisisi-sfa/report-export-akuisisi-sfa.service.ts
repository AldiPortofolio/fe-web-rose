import { Injectable } from '@angular/core';
import { HttpClient, HttpResponse } from '@angular/common/http';
import { tap } from 'lodash';
import { Observable } from 'rxjs';
import { SERVER_GO } from 'src/app/shared/constants/base-constant';
import { ReportExportAkuisisiSfa } from './report-export-akuisisi-sfa.model';

@Injectable({
  providedIn: 'root'
})
export class ReportExportAkuisisiSfaService {

  private serverUrl = SERVER_GO + 'report-export-akuisisi-sfa';

  constructor(
    private http: HttpClient
  ) { }

  filter(req?: any): Observable<HttpResponse<ReportExportAkuisisiSfa[]>> {
    let url = this.serverUrl + '/filter';

    console.log('CONSOLE LOG', req);

    return this.http.post<ReportExportAkuisisiSfa[]>(url, req, { observe: 'response' })
    
    
  }

  async download(req?: any): Promise<HttpResponse<Blob>> {

    const newresourceUrl = this.serverUrl + '/download';

    return await this.http.post<Blob>(
      newresourceUrl, req,
      { responseType: 'blob' as 'json', observe: 'response' }
    ).toPromise();
  }
  
}
