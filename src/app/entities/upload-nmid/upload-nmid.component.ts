...
@Component({
  selector: 'op-upload-nmid',
  templateUrl: './upload-nmid.component.html',
  styleUrls: ['./upload-nmid.component.css']
})
export class UploadNmidComponent implements OnInit {

    reportList: UploadNmid[];
    curPage = 1;
    totalData = 0;
    totalRecord = 10;
    ...
    constructor(
        ...
        private uploadNmidService: UploadNmidService,
    ...
    loadAll(page) {
        ...
        this.uploadNmidService.filter({
            ...
            .subscribe(
                (res: HttpResponse<UploadNmid[]>) => this.onSuccessGetData(res.body),
    ...
    submitUpload() {
        ...
        this.uploadNmidService.uploadFile(formData)
        .subscribe(
            (res: HttpResponse<UploadNmid>) => this.onSuccess(res),
            (res: HttpErrorResponse) => this.onError(res.message),
            // result => {
    ...
    downloadFile(filePath) {
        ...
        this.uploadNmidService.downloadFile({
            filePath: filePath
        }).then(
                (resp) => {
    ...
    downloadResultFile(filePath) {
        ...
        this.uploadNmidService.downloadResultFile({
            filePath: filePath
        }).then(
            (resp) => {
                console.log('res-->', resp);
