...
import { WilayahConfigurationModule } from './wilayah-configuration.module';
...
export class WilayahConfigurationService {
    ...
    getFromMda(id): Observable<HttpResponse<WilayahConfiguration[]>> {
        const newresourceUrl = this.serverUrl + `/getFromMda/${id}`;
        return this.http.get<WilayahConfiguration[]>(newresourceUrl, { observe: 'response' })
            .pipe(
                tap(result => console.log('YEAYYY hasil', result))
            );
    }

    approveFromMda(id): Observable<WilayahConfiguration> {
        const newresourceUrl = this.serverUrl + `/approveMda/${id}`;
        return this.http.post<WilayahConfiguration>(newresourceUrl, { observe: 'response' });
    }
