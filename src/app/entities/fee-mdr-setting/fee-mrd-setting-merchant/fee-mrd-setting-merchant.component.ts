...
@Component({
  selector: 'op-fee-mrd-setting-merchant',
  templateUrl: './fee-mrd-setting-merchant.component.html',
  styleUrls: ['./fee-mrd-setting-merchant.component.css']
})
export class FeeMrdSettingMerchantComponent implements OnInit {
  ...
  feeMdrList: FeeMdrSettingMerchant[];
  feeMdr: FeeMdrSettingMerchant;
  closeResult: string;
  currPage = 1;
  totalRecord = 10;
  totalData = 0;
  ...
  constructor(
    private feeMrdSettingService: FeeMrdSettingMerchantService,
  ...
  loadAll(page) {
    ...
    this.feeMrdSettingService.filter({
      ...
      .subscribe(
        (res: HttpResponse<FeeMdrSettingMerchant[]>) => this.onSuccess(res.body, res.headers),
  ...
  open(status, message) {
    const modalRef = this.modalService.open(FeeMdrSettingMerchantModalComponent, { size: 'lg' });
